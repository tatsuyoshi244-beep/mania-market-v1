...
create index if not exists partner_applications_pending_owner_email_idx
  on public.partner_applications (lower(pending_owner_email))
...
-- 既存の公開済み・オーナー未設定データを補完
update public.shops s
set pending_owner_email = pa.email
from public.partner_applications pa
where pa.shop_id = s.id
  and s.owner_id is null
  and s.pending_owner_email is null
  and pa.status = 'published';

update public.partner_applications pa
set pending_owner_email = pa.email
where pa.status = 'published'
  and pa.shop_id is not null
  and pa.pending_owner_email is null
  and exists (
    select 1 from public.shops s
    where s.id = pa.shop_id and s.owner_id is null
  );